...
for (let i = 0; i < Productos.length; i++) {
    ...
    lista.innerHTML += `
        <div onclick="mostrar(this.id)" id="${Productos[i].id}"  class=" producto col-12 col-md-12 col-lg-12 d-flex align-items-center">

            <img src="${Productos[i].imagen}" alt="">
            <div>
                
                <p>${Productos[i].nombre}</p>
                <p>$${Productos[i].precio.toLocaleString(`es-AR`)}</p>
        ...
        if (existente) {
            ...
            filas.forEach(fila => {
                if (fila.children[1].textContent === producto.nombre) {
                    const spanCantidad = fila.querySelector(".cantidad-span");
                    const tdSubtotal = fila.querySelector(".sub_total");
                    spanCantidad.textContent = existente.cantidad;
...
function generar_producto(id) {
    ...
    tr.innerHTML = `
        ...
        <td class="celda-cantidad"><span class="cantidad-span">1</span></td>
